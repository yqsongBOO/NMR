...
# from torch.optim.lr.scheduler import LinearLR

from models.molecularEncodeGT import molecularGT
...
class DiscreteDenoisingDiffusionMolecular_condition(pl.LightningModule):
    ...
    def training_step(self, data, i):
        ...
        if self.current_epoch == 1:
            batch_X = torch.argmax(X, dim=-1)
            batch_E = torch.argmax(E, dim=-1)
            n_nodes = data.atom_count
            batch_length = data.num_graphs
            molecule_list = []
            for i in range(batch_length):
                n = n_nodes[i]
                atom_types = batch_X[i, :n].cpu()
                edge_types = batch_E[i, :n, :n].cpu()
                molecule_list.append([atom_types, edge_types])
            for i in range(batch_length):
                mol = self.mol_from_graphs(molecule_list[i][0].numpy(), molecule_list[i][1].numpy())
                smiles = Chem.MolToSmiles(mol, isomericSmiles=True)
                self.smilesTrainVal.append(smiles)

        noisy_data = self.apply_noise(X, E, data.y, node_mask)
        extra_data = self.compute_extra_data(noisy_data)
        pred, molecularVec = self.forward(noisy_data, extra_data, node_mask, X, E)
        if self.current_epoch == 1:
            for i in range(molecularVec.shape[0]):
                vector = molecularVec[i]  # 提取第 i 个向量
                self.molecularVector.append(vector)  # 添加到列表中
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        if self.current_epoch== 1:
            # 创建一个数据框架
            data_ = {
                "smiles": self.smilesTrainVal,
                "molecularRep": self.molecularVector
            }
            # data = {key: value.cpu().numpy() if isinstance(value, torch.Tensor) else value for key, value in data.items()}
            # data = {key: value.cpu().tolist() if isinstance(value, torch.Tensor) else value for key, value in data.items()}
            ...
            data_ = move_to_cpu(data_)
            df = pd.DataFrame(data_)

            # 将数据保存为 CSV 文件
            csv_file_path = '/public/home/ustc_yangqs/molecular2molecular/src/mol_rep_25.csv' # 指定文件路径
    ...
    def test_step(self, data, i):
        ...
        pred, molecularVec  = self.forward(noisy_data, extra_data, node_mask, dense_data.X, dense_data.E)
    ...
    def reconstruction_logp(self, t, X, E, node_mask, condition):
        ...
        extra_data = self.compute_extra_data(noisy_data)
        pred0, molecularVec  = self.forward(noisy_data, extra_data, node_mask, X, E)
    ...
    def sample_batch(self, batch_id: int, batch_size: int, batch_condition, keep_chain: int, number_chain_steps: int,
        ...
        if keep_chain > 0:
            ...
            # print(f'chain_X[0]:{chain_X[0].shape}final_X_chain:{final_X_chain.shape}')
    ...
    def sample_p_zs_given_zt(self, s, t, X_t, E_t, y_t, node_mask, conditionVec, batch_X, batch_E):
        ...
        extra_data = self.compute_extra_data(noisy_data)
        pred, molecularVec = self.forward_sample(noisy_data, extra_data, node_mask, batch_X, batch_E)
