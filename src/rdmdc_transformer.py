...
class DiscreteDenoisingDiffusionCondition(pl.LightningModule):
    def __init__(self, cfg, dataset_infos, train_metrics, sampling_metrics, visualization_tools, extra_features,
        ...
        import pandas as pd
        # 加载 CSV 文件
        csv_path = "/public/home/ustc_yangqs/molecular2molecular/src/mol_rep_20.csv"  # 替换为你的 CSV 文件路径
    ...
    def on_test_epoch_end(self) -> None:
        ...
        for repeat in range(1):
            ...
            while samples_left_to_generate > 0:
                ...
                for i in range(to_generate):
                    mol = self.mol_from_graphs(molecule_list_picklist[max_indices[i]][i][0].numpy(),
                                               molecule_list_picklist[max_indices[i]][i][1].numpy())
                    mol_true = self.mol_from_graphs(molecule_list_True[i][0].numpy(), molecule_list_True[i][1].numpy())
                    try:
                        # 尝试获取 SMILES 表示
            ...
            # df.to_csv(
            #     '/public/home/ustc_yangqs/molecular2molecular/src/similarity_results_Retrieval_CH_25_top1_without_formular.csv',
            #     index=False)
    ...
    @torch.no_grad()
    def sample_batch_notrie(self, batch_id: int, batch_size: int, batch_condition, keep_chain: int, number_chain_steps: int,
        ...
        for s_int in reversed(range(0, self.T)):
            ...
            sampled_s, discrete_sampled_s = self.sample_p_zs_given_zt(s_norm, t_norm, X, E, y, node_mask,
                                                                      conditionVec=batch_condition, batch_X=batch_X,
                                                                      batch_E=batch_E)
    ...
    def sample_batch(self, batch_id: int, batch_size: int, batch_condition, keep_chain: int, number_chain_steps: int,
        ...
        molecule_list_True_1 = []
        batch_X_1 = torch.argmax(batch_X, dim=-1)
        batch_E_1 = torch.argmax(batch_E, dim=-1)
        for i in range(batch_size):
            n = n_nodes[i]
            atom_types_true_1 = batch_X_1[i, :n].cpu()
            edge_types_true_1 = batch_E_1[i, :n, :n].cpu()
            molecule_list_True_1.append([atom_types_true_1, edge_types_true_1])
        true_smiles = []
        for i in range(batch_size):
            mol_true_1 = self.mol_from_graphs(molecule_list_True_1[i][0].numpy(), molecule_list_True_1[i][1].numpy())
            smiles_1 = Chem.MolToSmiles(mol_true_1, isomericSmiles=True)
            true_smiles.append(smiles_1)

        # # 假设你的 CSV 文件路径为 'your_file.csv'
        # csv_file = '/public/home/ustc_yangqs/molecular2molecular/src/smiles_true_pred.csv'
        #
        # df = pd.read_csv(csv_file)
        #
        # result_smiles = []
        #
        # for smile in true_smiles:
        #     # 在 CSV 的 smiles_true 列中查找匹配的 SMILES
        #     match = df[df['smiles_true'] == smile]
        #
        #     # 如果找到匹配的行
        #     if not match.empty:
        #         # 获取对应的 smiles_pred 值并添加到列表
        #         result_smiles.append(match['smiles_pred'].iloc[0])
        #     else:
        #         # 如果没有找到匹配的 SMILES，可以选择添加 None 或空字符串
        #         print('no match')
        #         result_smiles.append(None)  # 或者 pred_smiles.append('') 根据需求
        node_list = []
        adj_matrix_list = []
        ...
        chain_X_size = torch.Size((int(number_chain_steps * 15 / 50), keep_chain, X.size(1)))
        chain_E_size = torch.Size((int(number_chain_steps * 15 / 50), keep_chain, E.size(1), E.size(2)))
        ...
        # Iteratively sample p(z_s | z_t) for t = 1, ..., T, with s = t - 1.
        for s_int in reversed(range(0, int(self.T * 15 / 50))):
            s_array = s_int * torch.ones((batch_size, 1)).type_as(y)
            t_array = s_array + 1
            s_norm = s_array / self.T
            t_norm = t_array / self.T
        ...
        if keep_chain > 0:
            ...
            chain_X = torch.cat([chain_X, chain_X[-1:].repeat(10, 1, 1)], dim=0)
            chain_E = torch.cat([chain_E, chain_E[-1:].repeat(10, 1, 1, 1)], dim=0)
            assert chain_X.size(0) == (number_chain_steps*15/50 + 10)
