...
class DiscreteDenoisingDiffusionCondition(pl.LightningModule):
    def __init__(self, cfg, dataset_infos, train_metrics, sampling_metrics, visualization_tools, extra_features,
        ...
        import pandas as pd
        # 加载 CSV 文件
        csv_path = "/public/home/ustc_yangqs/molecular2molecular/src/mol_rep_15.csv"  # 替换为你的 CSV 文件路径
            ...
            while samples_left_to_generate > 0:
                ...
                for i in range(to_generate):
                    ...
                    try:
                        ...
                        samples_true_pred.append([smiles2, smiles1])
                ...
                chains_left_to_save -= chains_save
                sys.stdout.flush()
            # df = pd.DataFrame(samples_true_pred, columns=["smiles_true", "smiles_pred"])
            # df.to_csv("smiles_true_pred.csv", index=False)
    ...
    def sample_batch(self, batch_id: int, batch_size: int, batch_condition, keep_chain: int, number_chain_steps: int,
        ...
        for i in range(batch_size):
            ...
            print(f'result_smiles[{i}]:{result_smiles[i]}')
        ...
        for s_int in reversed(range(0, int(self.T * 19 / 50))):
            ...
            # print(f'------------Xsize{X.shape} Esize{E.shape} ysize{y.shape} node_mask{node_mask.shape} ---------------')
            X, E, y = sampled_s.X, sampled_s.E, sampled_s.y
