...
class contrast_M_nmr(pl.LightningModule):
    def __init__(self, cfg, dataset_infos, train_metrics, sampling_metrics, visualization_tools, extra_features,
        ...
        self.model = contrastGT(n_layers_GT=cfg.model.n_layers,
                                      ...
                                      act_fn_in=nn.ReLU(),
                                      act_fn_out=nn.ReLU(),
                                      enc_voc_size=self.enc_voc_size, max_len=self.max_len, d_model=self.d_model, ffn_hidden=self.ffn_hidden, n_head=self.n_head, n_layers_TE=self.n_layers_TE, drop_prob=self.drop_prob, device=device)
    ...
    def training_step(self, data, i):
        ...
        Vec = predV1 - predV2
        # print(f'trainVec{Vec}')
        # print(f'trainVec_shape{Vec.shape}')
        # 对每个样本的各项平方求和，得到一个形状为 (batchsize, ) 的张量
        Vec_squared_sum = Vec.pow(2).sum(dim=1)  # 或者 Vec ** 2 后再 sum(dim=1)
        # 对所有样本的平方和求平均
        loss = Vec_squared_sum.mean()
        # cosine_similarities = F.cosine_similarity(predV1, predV2, dim=-1)
        # loss = (1 - cosine_similarities).sum()
    ...
    def validation_step(self, data, i):
        ...
        # print(f'valVec{Vec}')
        # print(f'valVec_shape{Vec.shape}')
        # 对每个样本的各项平方求和，得到一个形状为 (batchsize, ) 的张量
        Vec_squared_sum = Vec.pow(2).sum(dim=1)  # 或者 Vec ** 2 后再 sum(dim=1)
    ...
    def on_test_epoch_end(self) -> None:
        ...
        # # self.test_atomCount = self.test_atomCount.view(-1, 1)
        ...
        #     batch_X = self.test_data_X[start_index:start_index + to_generate]
        #     batch_E = self.test_data_E[start_index:start_index + to_generate]
        #
        #     samples.extend(self.sample_batch(batch_id=ident,
        #                                      batch_size=to_generate,
        #                                      num_nodes=batch_atomCount,
    ...
    #     """
    #     print(f'sample_batch{batch_X.shape}')
    ...
    #     extra_data = self.compute_extra_data(noisy_data)
    #     print(f'sample_p_zs_given_zt_batch_X{batch_X.shape}')
