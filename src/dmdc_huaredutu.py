...
class DiscreteDenoisingDiffusionCondition(pl.LightningModule):
    ...
    def on_test_epoch_end(self) -> None:
        ...
        for repeat in range(1):
            ...
            while samples_left_to_generate > 0:
                ...
                with torch.no_grad():
                    ...
                    for i in range(1):
                        ...
                        output_mol_true = molVec(node_mask, batch_x, batch_e)
                        mol_vec.append(output_mol_true)
                        mol_vec_list.append(output_mol)
                        nmr_tensor = torch.stack(nmr_vec, dim=0)
                        mol_tensor = torch.stack(mol_vec, dim=0)
                        nmr_array = nmr_tensor.cpu().numpy()
                        mol_array = mol_tensor.cpu().numpy()
                        print('------------------------')
                        sys.stdout.flush()
                        data = {
                            'nmr': [nmr_array[i].tolist() for i in range(nmr_array.shape[0])],
                            'mol': [mol_array[i].tolist() for i in range(mol_array.shape[0])]
                        }
                        ...
                        # 保存到 CSV 文件
                        df.to_csv('/public/home/ustc_yangqs/molecular2molecular/src/huaredutu_2.csv', index=False)
                        break
                break
            # nmr_tensor = torch.stack(nmr_vec, dim=0)
            # mol_tensor = torch.stack(mol_vec, dim=0)
            # nmr_array = nmr_tensor.numpy()
            # mol_array = mol_tensor.numpy()
            #
            # data = {
            #     'nmr': [nmr_array[i].tolist() for i in range(nmr_array.shape[0])],
            #     'mol': [mol_array[i].tolist() for i in range(mol_array.shape[0])]
            ...
            #
            # # 保存到 CSV 文件
            # df.to_csv('huaredutu.csv', index=False)
        ...
        #
        #         for i in range(to_generate):
        #             mol = self.mol_from_graphs(molecule_list_picklist[max_indices[i]][i][0].numpy(), molecule_list_picklist[max_indices[i]][i][1].numpy())
        #             mol_true = self.mol_from_graphs(molecule_list_True[i][0].numpy(), molecule_list_True[i][1].numpy())
        #             try:
        #                 # 尝试获取 SMILES 表示
        ...
        #         for i in range(to_generate):
        #             for k in range(5):
        #                 mol = self.mol_from_graphs(molecule_list_picklist[top5_indices[k][i]][i][0].numpy(), molecule_list_picklist[top5_indices[k][i]][i][1].numpy())
        #                 mol_true = self.mol_from_graphs(molecule_list_True[i][0].numpy(), molecule_list_True[i][1].numpy())
        #                 try:
        #                     # 尝试获取 SMILES 表示
        #                     smiles1 = Chem.MolToSmiles(mol, isomericSmiles=True)
        ...
        #         for i in range(to_generate):
        #             for k in range(10):
        #                 mol = self.mol_from_graphs(molecule_list_picklist[top10_indices[k][i]][i][0].numpy(), molecule_list_picklist[top10_indices[k][i]][i][1].numpy())
        #                 mol_true = self.mol_from_graphs(molecule_list_True[i][0].numpy(), molecule_list_True[i][1].numpy())
        #                 try:
        #                     # 尝试获取 SMILES 表示
        #                     smiles1 = Chem.MolToSmiles(mol, isomericSmiles=True)
        ...
        # df.to_csv('/public/home/ustc_yangqs/molecular2molecular/src/similarity_results_noRetrieval_H_25_top1_without_formular.csv', index=False)
